...
The answer is stark, and it arrives fast. A realistic attacker — one who reads past surface form (so `aged 47` and `47-year-old` are the same fact) and starts with the *rarest* fact rather than the first one in the document — singles out **81% of documents from a single quasi-identifier**, and **96.5% within three**. Only **12 of 1,268 documents (0.9%)** are never unique, hidden because another file's facts happen to be a superset of theirs; all the rest are uniquely identifiable from their quasi-identifiers alone, even after a perfect DIRECT-identifier redaction. And *which* facts you know matters as much as how many: revealing them in the order they appear instead needs **seven** facts to reach that same 95%.

(This is the *hardened* version of the original analysis, and the upgrade is the point. The first cut matched fingerprints on exact surface form and revealed facts in document order — both understated the risk. Folding surface variants together (the conservative choice: it can only *reduce* apparent uniqueness) plus an attacker who picks the most-discriminating facts first turns the earlier hand-wave into a measured sensitivity analysis. The greedy attacker is a tested helper, `anonymisation.mosaic.min_facts_to_identify`. One bound remains: the haystack is TAB itself, so a real attacker's comparison corpus is larger and noisier than 1,268 ECHR cases.)
...
2. **The haystack is TAB, not a firm's corpus.** *(Partly addressed.)* Fingerprint matching now folds surface variants (`47-year-old` ≈ `forty-seven-year-old`) and models an attacker who combines the most-discriminating facts first, so the *within-TAB* numbers are honest rather than a strict-surface-form bound. What's still a stand-in is the comparison corpus itself: a real attacker matches against a larger, noisier population than 1,268 ECHR cases, which would move the absolute rates. Validating against a firm-scale haystack is the open item.
