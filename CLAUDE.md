...
Deciding *which* mentions share a token is a measured decision, not a heuristic (notebook 17). `classify_link_evidence` names the rule that fired — exact repeat, honorific-stripped equality, initialism (`World Health Organisation` … `WHO`) in either direction, containment in either direction — and `LINK_CONFIDENCE` holds that rule's precision per entity type, fitted by `scripts/evaluate_coref_links.py` on TAB train+validation. `Pseudonymiser` merges only above `min_link_confidence` (0.5) and refuses a merge when two entities match equally well; every resolution is recorded as a `LinkDecision` on `result.pseudonym_links`. `link_policy="legacy"` reproduces the old first-match-wins behaviour for comparison, and `disabled_evidence` turns a single rule off so its contribution can be costed.

**Coreference is only attempted for PERSON and ORG** (`LINKABLE_TYPES`, overridable via `coref_types`). Every other type links on an exact repeat of the surface form and nothing else — that is identity, not coreference. This is a scope decision, not a free one: it costs 1.4 points of pairwise recall, all of it MISC and DEM initialisms, and notebook 17 reports the cost. Do not widen it to chase recall without a reason to believe a descriptor is an identity.

**Do not raise the threshold to maximise pairwise F1** — F1 peaks at 0.30 and the default is deliberately 0.50, because a false merge makes `restore()` write the wrong person's name into the output while a missed merge only costs context. The intended workflow: redact locally → send pseudonymised text to an external LLM → `restore()` the LLM's answer locally using the vault, so real names never leave the firm. CLI: `redact --pseudonymise --vault-out vault.json`, then `restore --vault vault.json`.
